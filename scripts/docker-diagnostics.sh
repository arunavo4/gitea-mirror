...
echo -e "1. ${GREEN}./scripts/build-docker.sh --load${NC} (build and load into Docker)"
echo -e "2. ${GREEN}docker-compose up -d${NC} (start the container)"
...
echo -e "1. Check container logs: ${GREEN}docker logs gitea-mirror${NC}"
echo -e "2. Check health status: ${GREEN}docker inspect --format='{{.State.Health.Status}}' gitea-mirror${NC}"
